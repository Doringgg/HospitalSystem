...
class MyTokenJWT {
    ...
    public function generateToken(stdClass $claims): string
    {
        ...
        $objHeaders->alg = MyTokenJWT::ALGORITHM;
        $objHeaders->typ = MyTokenJWT::TYPE;
    ...
    public function validateToken($stringToken): bool {
        ...
        $patern = '/^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/';
        if (!preg_match($patern, $stringToken) === 1){
            return false;
        }
        $token = str_replace(["Bearer ", " "], "", $stringToken);
        try {
            $payloadValido = JWT::decode(jwt: $token, keyOrKeyArray: new Key(keyMaterial: MyTokenJWT::KEY, algorithm: MyTokenJWT::ALGORITHM));
